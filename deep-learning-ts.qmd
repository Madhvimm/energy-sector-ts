...
### Hyper parameter tuning to get the best parmeters
...
# def get_model(optimizer="adam", units=50, L2=0.01):
#     model = Sequential()
#     model.add(LSTM(units, input_shape=(lookback, features), activation='relu', 
#                    recurrent_regularizer=regularizers.l2(L2)))
#     model.add(Dense(1, activation='linear'))
#     model.compile(loss="mse", optimizer=optimizer)
#     return model
...
Based on the results , we can compare the performance of the three different ANN models (RNN, LSTM, and GRU) with and without regularization for crude oil data:

1. LSTM with Regularization:
   - The LSTM model with regularization achieves the lowest validation RMSE of 0.731646 among all the models.
   - It also has a relatively low train RMSE of 1.123175, indicating good performance on both the training and validation sets.
   - This suggests that the LSTM model with regularization has the highest accuracy and predictive power for the crude oil data.

2. GRU with Regularization:
   - The GRU model with regularization has the second-lowest validation RMSE of 0.777009.
   - Its train RMSE is 1.054538, which is slightly lower than the LSTM model with regularization.
   - The GRU model with regularization performs well but slightly less accurately than the LSTM model with regularization.

3. RNN without Regularization:
   - The RNN model without regularization has a validation RMSE of 0.840821, which is higher than the LSTM and GRU models with regularization.
   - Its train RMSE is 1.448784, indicating a larger difference between the training and validation performance compared to the regularized models.
   - The RNN model without regularization has lower accuracy and predictive power compared to the regularized LSTM and GRU models.

4. LSTM without Regularization:
   - The LSTM model without regularization has a validation RMSE of 0.843815, similar to the RNN model without regularization.
   - Its train RMSE is 1.441180, also close to the RNN model without regularization.
   - The LSTM model without regularization performs similarly to the RNN model without regularization, with slightly lower accuracy compared to the regularized models.

5. GRU without Regularization:
   - The GRU model without regularization has a validation RMSE of 0.848463, which is slightly higher than the LSTM and RNN models without regularization.
   - Its train RMSE is 1.441131, similar to the other non-regularized models.
   - The GRU model without regularization has the lowest accuracy among the non-regularized models.

6. RNN with Regularization:
   - The RNN model with regularization has the highest validation RMSE of 1.171465 among all the models.
   - Its train RMSE is 1.443878, indicating a significant difference between the training and validation performance.
   - The RNN model with regularization performs the worst in terms of accuracy and predictive power for the crude oil data.

In summary, the LSTM model with regularization demonstrates the highest accuracy and predictive power, followed by the GRU model with regularization. The non-regularized models (RNN, LSTM, and GRU) have lower accuracy compared to their regularized counterparts. The RNN model with regularization performs the worst among all the models.

These results suggest that incorporating regularization techniques can improve the performance of ANN models for crude oil price prediction, with LSTM and GRU models benefiting more from regularization compared to the simple RNN model.
