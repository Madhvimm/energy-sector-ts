...
These accuracy measures indicate that the ARIMA(2,1,3) model is providing forecasts that are consistent with the historical data, not deviating significantly from the patterns already observed, and is performing well against standard benchmarks.
...
# forecast next three years
fit_elec_s %>% forecast(h=36) %>% autoplot()+theme_bw() 
```
...
autoplot(electricity_log_ts) +
  ...
  autolayer(rwf(electricity_log_ts, h=36, drift=TRUE),
            series="Drift", PI=FALSE)+
  autolayer(forecast(fit_elec_s,36), 
            series="fit",PI=FALSE) +
  guides(colour=guide_legend(title="Forecast")) +theme_bw() 
```
...
#n=length(electricity_log_ts)
#n-k=50; 96/12=5; 
k=51
mae1 <- matrix(NA, 5,12) # change to 5
mae2 <- matrix(NA,5,12)

st <- tsp(electricity_log_ts)[1]+(k-1)/12 #48 observations
# put up to 5 because 96/12 = 5
for(i in 1:5)
{
  #xtrain <- window(a10, start=st+(i-k+1)/12, end=st+i/12)
