...
gas_log_ts <- ts(citygate_gas_prices$LOG_Value, start=c(start_year, start_month), frequency=12)
...
electricity_log_ts <- ts(total_electricity_prices$LOG_Value, start=c(start_year, start_month), frequency=12)
...
p = 1, 2, 3, 4 (considering the significant spike at the first lag)
q = 1, 2, 3, 4, 5, 6, 7, 8, 9
...
temp <- data.frame()
ls <- matrix(rep(NA, 6 * 15), nrow = 15)  # 15 combinations
...
- Best Model by AIC (and AICc) and BIC is : ARIMA(2,1,0)
- Second Best would be (2,1,1)
...
d <- 1 # Degree of differencing
temp <- data.frame()
ls <- matrix(rep(NA, 6 * 26), nrow = 26)  # p (4 options) * q (9 options) = 36 combinations
...
- Best Model by AIC, BIC, and AICc (ARIMA(2,1,7))
- ARIMA(3,1,2)
...
#Fitting ARIMA
fit_crudeoil <- Arima(diff(oil_log_ts), order=c(2, 1, 0),include.drift = TRUE) 
summary(fit_crudeoil)
```

**Coefficients:** The model includes two autoregressive terms (ar1, ar2) and zero moving average terms, with all coefficients having significant values.
**Drift:** The drift coefficient is effectively zero, suggesting no consistent linear trend in the differenced series.


For the Best Model (ARIMA(2,1,0)):
...
The equation for the ARIMA(2,1,0) model, ignoring the drift since it's 0, would be:

$$
y_t = -0.1203y_{t-1} -0.2649y_{t-2} + w_t 
$$
...
**Coefficients:** The model includes three autoregressive terms (ar1, ar2) and one moving average terms (ma1), with all coefficients having significant values.
**Drift:** The drift coefficient is effectively zero, suggesting no consistent linear trend in the differenced series.
...
$$
y_t = -0.2333y_{t-1} - 0.1575y_{t-2} - 0.0137y_{t-3} + w_t - 1.0w_{t-1}
$$
...
#Fitting ARIMA
fit_electricity <- Arima(diff(electricity_log_ts), order=c(2, 1, 7),include.drift = TRUE) 
summary(fit_electricity)

```

**Coefficients:** The model includes two autoregressive terms (ar1, ar2) and seven moving average terms with all coefficients having significant values.
**Drift:** The drift coefficient is extremely small, suggesting a negligible linear trend in the differenced series.


For the Best Model (ARIMA(2,1,7)):
...
The equation for the ARIMA(2,1,7) model:

$$
y_t = 0.8665y_{t-1} -0.8255y_{t-2}+ w_t -1.5081w_{t-1} + 1.1396w_{t-2} - 0.6100w_{t-3} - 0.2225w_{t-4} + 0.0819w_{t-5} -0.0818w_{t-6} +  0.2288w_{t-7}
$$
...
$$
y_t = 0.5077y_{t-1} + w_t - 1.2832w_{t-1} + 0.3072w_{t-2}
$$
...
$$
y_t = - 0.6173y_{t-1} - 0.7741y_{t-2} + 0.2142y_{t-3} + w_t + 0.0696w_{t-1} + 0.2021w_{t-2}-0.8341w_{t-3}-0.0649w_{t-4}
$$
...
#Fitting ARIMA
fit_crudeoil2 <- Arima(diff(oil_log_ts), order=c(2, 1, 1),include.drift = TRUE) 
...
$$
y_t = 0.5844y_{t-1} - 0.2242y_{t-2} + w_t - 1.00 w_{t-1}
$$



Given these points, the ARIMA(2,1,1) model appears to have a slight edge in terms of the goodness of fit (based on AIC and error metrics).
...
$$
x_t = - 0.1999x_{t-1} + w_t - 1.0w_{t-1}
$$
...
#Fitting ARIMA
fit_electricity2 <- Arima(diff(electricity_log_ts), order=c(3, 1, 2),include.drift = TRUE) 
summary(fit_electricity2)

```

**Coefficients:** The model includes three autoregressive terms (ar1, ar2) and two moving average terms (ma1, ma2), with all coefficients having significant values.
**Drift:** The drift coefficient is extremely small, suggesting a negligible linear trend in the differenced series.
...
$$
x_t = 1.2344x_{t-1} - 0.4796x_{t-2} - 0.1080 x_{t-3} + w_t - 1.8302w_{t-1} + 0.8422w_{t-2}
$$
...
set.seed(222)
diag_crudeoil <- capture.output(sarima(oil_log_ts, 2,1,0))
...
The diagnostic plots suggest that the ARIMA(2,1,0) model is a reasonably good fit for the data.
...
set.seed(222)
diag_electricity <- capture.output(sarima(electricity_log_ts, 2,1,7))
...
We used auto.arima() to pick the best model, and it chose ARIMA(2,1,0). Interestingly, this is the same model we thought was best earlier. 
So, while it's great that auto.arima() confirmed our model choice, we should be a bit cautious. It's always good to double-check the model's predictions and not rely solely on what the automated tool suggests.
