...
Given these points, the ARIMA(2,1,4) model appears to have a slight edge in terms of the goodness of fit (based on AIC and error metrics).
...
**Ljung-Box test**: While the p-values at various lags are notably low, suggesting the presence of significant autocorrelation, this does not align with the ACF plot which did not show such correlations. 


The diagnostic plots suggest that the ARIMA(2,1,4) model is a reasonably good fit for the data. The residuals are mostly random, implying that the model is capturing the underlying process well. The Q-Q plot does suggest some minor issues with normality, particularly with possible outliers or extreme values. The presence of significant autocorrelation as indicated by the low p-values in the Ljung-Box test means the model may need to be improved. 
...
We used auto.arima() to pick the best model, and it chose ARIMA(2,1,4). Interestingly, this is the same model we thought was best earlier. 
