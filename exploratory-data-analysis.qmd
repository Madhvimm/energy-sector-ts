...
# Creating the log of GDP
log_gdp <- log(gdp_data$GDP)
gdp_ts <- ts(log_gdp, start=c(start_year_gdp, start_quarter_gdp), frequency=4)
...
# Creating the log of CPI
log_cpi <- log(cpi_data$CPIAUCSL)
cpi_ts <- ts(log_cpi, start = c(start_year_cpi, start_month_cpi), frequency = 12)
...
# Detrending using the original data frame
fit_log_cpi <- lm(log(CPIAUCSL) ~ DATE, data = cpi_data, na.action = NULL)
...
# Calculate moving averages with the specified window sizes
gdp_data$ma5_log <- rollmean(log(gdp_data$GDP), 5, fill = NA, align = "right", by = 4)  # Considering quarterly data
gdp_data$ma15_log <- rollmean(log(gdp_data$GDP), 15, fill = NA, align = "right", by = 4)
gdp_data$ma20_log <- rollmean(log(gdp_data$GDP), 20, fill = NA, align = "right", by = 4)
gdp_data$ma30_log <- rollmean(log(gdp_data$GDP), 30, fill = NA, align = "right", by = 4)
...
p_gdp <- ggplot(gdp_data, aes(x = DATE)) + 
  ...
  geom_line(aes(y = exp(ma5_log), color = "5-MA")) +
  geom_line(aes(y = exp(ma15_log), color = "15-MA")) +
  geom_line(aes(y = exp(ma20_log), color = "20-MA")) +
  geom_line(aes(y = exp(ma30_log), color = "30-MA")) +
...
# Calculate moving averages with the specified window sizes
cpi_data$ma5_log <- rollmean(log(cpi_data$CPIAUCSL), 5, fill = NA, align = "right")
cpi_data$ma15_log <- rollmean(log(cpi_data$CPIAUCSL), 15, fill = NA, align = "right")
cpi_data$ma20_log <- rollmean(log(cpi_data$CPIAUCSL), 20, fill = NA, align = "right")
cpi_data$ma30_log <- rollmean(log(cpi_data$CPIAUCSL), 30, fill = NA, align = "right")
